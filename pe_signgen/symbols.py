...
def _find_in_exports(pe: pefile.PE, func_name: str) -> tuple[int, str] | None:
    ...
    for sym in exports:
        ...
        for v in variant_lower:
            if (name_lower.startswith(v) or v.startswith(name_lower)) and (
                "@" in func_name
            ):
                return sym.address, name
...
def _find_in_pdb(pdb_path: str, func_name: str) -> tuple[int, int, int, str] | None:
    ...
    try:
        ...
        if result is not None:
            ...
            log_debug(
                f"PDB lookup success: {matched} at segment={segment} offset=0x{offset:x} rva=0x{rva:x}"
            )
...
def _rva_to_file_offset(pe: pefile.PE, rva: int) -> int:
    ...
    for section in pe.sections:
        ...
        if start <= rva < start + size:
            return int(section.PointerToRawData + (rva - start))
...
def find_function(
    ...
    if not pdb_path_obj.exists():
        raise SymbolNotFoundError(
            func_name, f"Not in exports and PDB not found at {pdb_path}"
        )
    ...
    if pdb_result is not None:
        ...
        if rva == 0 and segment > 0 and segment <= len(pe.sections):
            ...
            log_debug(
                f"Computed RVA 0x{rva:x} from segment {segment}, offset 0x{offset:x}"
            )
...
def get_all_symbols(pdb_path: str) -> list[Any]:
    """
